  ...
  return (
    <div className="class-tile" onClick={handleTileClick}>
      <div className="class-tile-header" style={{ background: color || '#764ba2' }}>
        <button className="class-tile-menu-btn" onClick={e => { e.stopPropagation(); setMenuOpen(p=>!p); }}>⋯</button>
        {menuOpen && (
          <div className="class-tile-dropdown" onClick={e => e.stopPropagation()}>
            <button onClick={e => { e.stopPropagation(); setMenuOpen(false); onEdit?.(); }}>Edit</button>
            <button className="delete" onClick={e => { e.stopPropagation(); setMenuOpen(false); onDelete?.(); }}>Delete</button>
          </div>
        )}
        <h3>{title}</h3>
        {classCode && <div style={{fontSize:11,color:'rgba(255,255,255,0.75)',marginTop:4,letterSpacing:1,fontFamily:'monospace'}}>{classCode}</div>}
      </div>
      <div className="class-tile-body">
        {courseName ? <div className="class-tile-course-name">{courseName}</div>
