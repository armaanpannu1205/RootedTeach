...
function ClassTile({ title, courseName, quarter, color, classId, onDelete, onEdit }) {
  const [menuOpen, setMenuOpen] = useState(false);
  const navigate = useNavigate();

  const handleTileClick = () => {
    navigate('/class', { state: { title, courseName, quarter, color, classId } });
  };
  ...
  return (
    <div className="class-tile" onClick={handleTileClick}>
      {/* Colored header with class code */}
      ...
      {/* Card body with course name + quarter */}
