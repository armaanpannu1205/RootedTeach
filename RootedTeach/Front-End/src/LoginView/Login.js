...
// Only init Firebase once even if component re-renders
...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Check if this person already has a Firestore profile
      const userRef = doc(db, "users", user.uid);
      const userSnap = await getDoc(userRef);

      if (!userSnap.exists()) {
        // New Google user — save them to Firestore with default Student role
